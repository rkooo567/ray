...
# SANG-TODO the issue is that raylet doesn't respect bundle index when it
# computes resource_ids. I think if we support 
def test_actor_cuda_visible_devices_placement_group_bundle_index(shutdown_only):
    ...
    assert ray.get(m2[0].get_cuda_visible_devices.remote()) == "1"
    # assert ray.get(m2[0].get_cuda_visible_devices.remote()) == "0"
    # assert ray.get(m2[1].get_cuda_visible_devices.remote()) == "1"


def test_actor_cuda_visible_devices_placement_group_bundle_index2(shutdown_only):
    ray.init(num_cpus=4, num_gpus=2)
